...
def import_fits_idi(basedir, fitsdir, workdir, prj, vis, idifiles):
    importfitsidi(fitsidifile=idifiles, vis=vis,
      constobsid=True,
...
def convert_flag(basedir, calibdir, workdir, prj, idifiles):
    ...
    fitsidi.convert_flags(AIPSflag, idifiles,
      outfile=outfile)
...
def set_working_vars(basedir, workdir, prj):
    ...
    """
    set_working_vars_base = lambda basedir, extradir, prj, extension: f"{basedir}/{extradir}/{prj}.{extension}"
...
def plotms_phase_freq(vis, ref='EF', field='0', avgtime='600'):
    plotms(vis=vis, xaxis="frequency", yaxis="phase", antenna=ref, ydatacolumn="corrected",
      iteraxis="baseline",
      correlation="ll,rr",
      coloraxis="spw",
      averagedata=True,
...
def plotms_freq_amplitude(vis, ref='EF', field='0', avgtime='600'):
    plotms(vis=vis, xaxis="frequency", yaxis="amplitude", antenna=ref, ydatacolumn="corrected",
      iteraxis="baseline",
      correlation="ll,rr",
      coloraxis="spw",
      averagedata=True,
...
def plotms_time_phase(vis, ref='EF', field='0', avgchannel='64'):
    plotms(vis=vis, xaxis="time", yaxis="phase", antenna=ref, ydatacolumn="corrected",
      iteraxis="baseline",
      correlation="ll,rr",
      coloraxis="scan",
      averagedata=True,
...
def plotms_time_amplitude(vis, ref='EF', field='0', avgchannel='64'):
    plotms(vis=vis, xaxis="time", yaxis="amp", antenna=ref, ydatacolumn="corrected",
      iteraxis="baseline",
      correlation="ll,rr",
      coloraxis="scan",
      averagedata=True,
...
def flagquack_intervals(vis):
    ...
    flagmanager(vis, mode="save", versionname="precal_flags", comment="Flags from Tsys, gaincal, bad data and edge channels")
